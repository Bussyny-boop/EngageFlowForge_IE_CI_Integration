...
public class DataUpdateGlobalEscalationTest {
    ...
    public void testGroupStateTreatedAsPrimary() throws Exception {
        ...
        for (ExcelParserV5.FlowRow flow : clinicals) {
            if ("VAssign:[Room] CHARGE NURSE".equals(flow.r1)) {
                foundChargeNurse = true;
                assertEquals("VMP", flow.deviceA, "Flow with CHARGE NURSE should use VMP");
            }
        }
        
        assertTrue(foundChargeNurse, "Should find flow with VAssign:[Room] CHARGE NURSE as R1 (Group state treated as Primary)");
    }
    ...
    public void testRecipientFallbackToDestination() throws Exception {
        ...
        for (ExcelParserV5.FlowRow flow : clinicals) {
            if (flow.r1 != null && !flow.r1.isEmpty()) {
                // Recipients should be one of: VAssign:[Room] NURSE, VAssign:[Room] NURSE BUDDY, VAssign:[Room] CHARGE NURSE, or a destination value
                assertTrue(
                    flow.r1.equals("VAssign:[Room] NURSE") || 
                    flow.r1.equals("VAssign:[Room] NURSE BUDDY") || 
                    flow.r1.equals("VAssign:[Room] CHARGE NURSE") ||
                    flow.r1.startsWith("#{"),
