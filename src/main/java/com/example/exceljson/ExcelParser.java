...
public class ExcelParser {
    ...
    /** NEW LOGIC — Reads “Facility” and “Common Unit Name” properly, gathers all config groups */
    private void parseUnitBreakdown() {
        Sheet sheet = wb.getSheet(config.sheets.unitBreakdown);
        if (sheet == null) return;

        int headerRowIdx = 1; // row index 1 = actual header row
    ...
    // -------------------- PARAMETER BLOCKS --------------------
    // (Same NurseCall + Clinical logic as before, omitted for brevity — unchanged)
package com.example.exceljson;
